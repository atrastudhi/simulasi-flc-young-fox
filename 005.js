...
function getGuildMemberInfo(members) {
  ...
  for(let i = 0; i < members.length; i++) {
    totalLv += members[i].level
  }

  let avarage = totalLv / members.length

  let result = {
    totalMember: members.length,
    avarageLevel: Math.floor(avarage)
  }

  for(let i = 0; i < members.length; i ++) {
    if(members[i].level > avarage) {
      if(result[members[i].class] === undefined) {
        result[members[i].class] = []
        let temp2 = {name: members[i].name, level: members[i].level}
        result[members[i].class].push(temp2)
      } else {
        let temp3 = {name: members[i].name, level: members[i].level}
        result[members[i].class].push(temp3)
      }
    }
  }
  if(members.length === 0) {
