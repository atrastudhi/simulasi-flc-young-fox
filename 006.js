...
function oddPairFirstLast(num) {
  ...
  let depanBelakang = String(num)[0] + String(num)[String(num).length-1]
  let returned = numStr.slice(1, numStr.length-1)
  if(num == 0) {
    return 0
  } else {
    if(numStr.length === 1 && Number(numStr + '1') % 2 !== 0) {
      return Number(numStr + '1') + oddPairFirstLast(0)
    } else if(numStr.length === 1 && Number(numStr + '1') % 2 === 0) {
      return oddPairFirstLast(0)
    } else if(Number(depanBelakang) % 2 !== 0) {
      return Number(depanBelakang) + oddPairFirstLast(Number(returned))
    } else {
      return oddPairFirstLast(Number(returned))
    }
  }
}
